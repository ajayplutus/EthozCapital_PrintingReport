...
function SaveNewInventory(url) {
    if (Validation()) {
        ...
        model.Id = $("#Id").val();
        model.Type = $("#txtTypeDescription").val();
        model.Value = $("#txtValue").val();
        ...
        $.post(varSitePath +"/Security/InsertInventoryDetails", { json: JSON.stringify(model) }, function (response) {
            $('#myModal').modal("hide");
...
function DischargeCancelStatus(url) {
    ...
    $.post(varSitePath +'/Security/UpdateInventoryStatus', { Id: Id, Status: Status }).done(function (response) {
        if (response.Status === 1) {
            swal({
