...
function getAddress(selectedCustomer) {
    $.get(varSitePath +"/Security/getAddress", { selected: selectedCustomer, IndividualCorporate: "Corporate" }, function (json) {
...
$(document).ready(function () {
    ...
    $('#txtCustomer').autocomplete({
        source: function (request, response) {
            $.get(varSitePath + "/Security/GetCustomerAutoComplete", { textFilter: request.term, IndividualCorporate: "Corporate" }, function (json) {
        ...
        select: function (event, ui) {
            ...
            $.get(varSitePath + "/Security/getRocUenType", { selected: ui.item.value }, function (json) {
...
function SaveNewCashandEquivalent(url) {
    ...
    if (Validation()) {
        ...
        $.post(varSitePath + "/Security/InsertCashEquivalentCompanyDetail", { json: JSON.stringify(model) }, function (response) {
...
function DischargeCancelStatus(url) {
    var Status = "C";
    var Id = $("#Id").val();
    $.post(varSitePath + '/Security/UpdateCashAndEquivalentComStatus', { Id: Id, Status: Status }).done(function (response) {
        if (response.Status === 1) {
            swal({
